    ...
    if (Array.isArray(driverActivity) && driverActivity.length > 0) {
      return [...driverActivity]
        ...
        .map((d) => ({
          ...
          status: String(d.status ?? '').toUpperCase() === 'AVAILABLE' ? 'available' : 'busy',
        }));
    }
    const counts = {};
    (missions ?? []).forEach((m) => {
...
/*function FuelSection({ coupons, driverActivity, missions }) {
  const fuel = useMemo(() => {
    ...
    // Top 5 by fuel consumed from driverActivity
    const top5fuel = [...(Array.isArray(driverActivity) ? driverActivity : [])]
      .filter((d) => Number(d.fuelUsed ?? d.totalFuel ?? d.couponsUsed ?? 0) > 0)
      .sort((a, b) =>
        Number(b.fuelUsed ?? b.totalFuel ?? b.couponsUsed ?? 0) -
        Number(a.fuelUsed ?? a.totalFuel ?? a.couponsUsed ?? 0)
      )
      .slice(0, 5)
      .map((d) => ({
        name: d.driverName ?? d.name ?? '—',
        used: Number(d.fuelUsed ?? d.totalFuel ?? d.couponsUsed ?? 0),
      }));
      ...
      <div className="card fuel-kpi-card">
        ...
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
            <span style={{ fontSize: 10, color: 'var(--dash-t2)', letterSpacing: 1, textTransform: 'uppercase' }}>Usage Rate</span>
            <span style={{ fontSize: 11, fontFamily: 'DM Mono, monospace', color: fuel.usagePct > 80 ? '#e74c3c' : '#2ecc71' }}>
              {fuel.usagePct}%
            </span>
          </div>
            ...
            {fuel.expiring.map((c, idx) => {
              ...
              return (
                <div className="expiry-row" key={c.id ?? idx} style={{ background: col.bg, borderColor: col.border }}>
                  ...
                  <div className="expiry-info">
                    <div className="dp-name" style={{ fontSize: 12 }}>{c.batchNumber ?? `Batch #${c.id}`}</div>
      ...
      <div className="card maint-summary-card">
        ...
        {maint.mostMaintained && (
          <div className="maint-most">
            ...
            <span style={{ color: 'var(--dash-t1)', fontFamily: 'DM Mono,monospace', fontSize: 12 }}>
              {maint.mostMaintained[0]}
            </span>
            <span style={{ color: 'var(--dash-t3)', fontFamily: 'DM Mono,monospace', fontSize: 11, marginLeft: 6 }}>
...
/*function ActivityFeed({ missions, maintenances, coupons }) {
  const feed = useMemo(() => {
    ...
    // Sort all by date desc, take top 8
    return items
          ...
          {feed.map((item, idx) => (
            <div className="activity-row" key={idx}>
              <div className="activity-icon-wrap" style={{ background: `${item.color}18`, border: `1px solid ${item.color}30` }}>
                <i className={`fas ${item.icon}`} style={{ color: item.color, fontSize: 13 }} />
              </div>
...
/*function Dashboard() {
  ...
  const computed = useMemo(() => {
    ...
    const pendingAlerts = [...maintenances, ...checks].filter((x) => {
      ...
      return s.includes('EXPIRE') || s.includes('EXPIR') || s.includes('WARNING') || s.includes('BREAK') || s.includes('MAINT');
    }).length;
    const fleetStatus = {
      active:      vehicles.filter((v) => String(v.status).toUpperCase() === 'ACTIVE').length,
      assigned:    vehicles.filter((v) => String(v.status).toUpperCase() === 'ASSIGNED').length,
      maintenance: vehicles.filter((v) => String(v.status).toUpperCase().includes('MAINT')).length,
      breakdown:   vehicles.filter((v) => String(v.status).toUpperCase().includes('BREAK')).length,
    };
    ...
    <PageLayout>
      <div className="dash">
        ...
        <div className="content-header">
          <div>
            <h2>Dashboard</h2>
            <span style={{ fontSize: 12, color: 'rgba(160,185,220,0.55)', letterSpacing: 1, textTransform: 'uppercase', fontFamily: "'DM Mono',monospace", marginTop: 4, display: 'block' }}>
        ...
        <div className="main-grid">
          <div className="card">
            ...
            <div className="donut-wrap">
              <svg className="donut-svg" width="110" height="110" viewBox="0 0 110 110">
                ...
                <text x="55" y="63" textAnchor="middle" fontFamily="Rajdhani,sans-serif" fontSize="8" fill="rgba(120,150,190,0.5)" letterSpacing="1.5">TOTAL</text>
              </svg>
          ...
          <div className="card span2">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ fontFamily: "'DM Mono',monospace", color: 'rgba(120,150,190,0.50)', fontSize: 11, letterSpacing: 0.6, textTransform: 'uppercase' }}>Missions by</div>
